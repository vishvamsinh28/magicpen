# MagicPen — Google Docs add-on

A Google Docs sidebar that edits the open document with AI, powered by the
MagicPen backend. This folder is a **separate Google Apps Script project** — it
is *not* part of the Next.js build. It talks to the MagicPen app over HTTPS.

```
Google Docs sidebar (this Apps Script)
        │  POST {APP_BASE_URL}/api/gdocs/assist   (header: X-MagicPen-Addon)
        ▼
MagicPen backend (Next.js)  ── runAssistant (Gemini) ── returns edited HTML
```
...
- The MagicPen backend deployed and reachable at a public HTTPS URL
...
1. **Create the Apps Script project and push this code:**
   ```bash
   cd google-addon
   clasp create --title "MagicPen" --type docs
...
2. **Configure it.** In the Apps Script editor: **Project Settings → Script
   ...
   - `APP_BASE_URL` = your MagicPen origin, no trailing slash (e.g. the ngrok URL)
...
3. **Install it for yourself.** In the editor: **Deploy → Test deployments →
   Install** (Editor Add-on). Open any Google Doc → **Extensions → MagicPen →
   Open MagicPen**.
...
2. Each teammate opens the script, then **Deploy → Test deployments → Install**
   for their own account. They now get **Extensions → MagicPen** in their Docs.
3. On first use each person clicks **Connect MagicPen** and links their own
   MagicPen account — after that it just works for them.
...
The first edit shows a **Connect MagicPen** link. Open it, sign in to your
MagicPen account, and confirm — this maps your Google email to your MagicPen
...
- `script.external_request` — call the MagicPen backend.
- `userinfo.email` + `openid` — identify the running user via a signed OpenID
  token (reliable across different Gmail accounts, to link MagicPen accounts).
...
- Stateless — no follow-up memory, versions, or undo yet (the doc is the source
  of truth). Those would need the add-on to bind the doc to a MagicPen
...
access); `[backend]` = the call actually reached MagicPen.
