...
/**
 * POST /api/chat — run one assistant turn and persist both sides of it.
 * The AI call happens BEFORE any writes, so a failed request leaves no
 * half-recorded chat; AI errors map to 400/429/502 with their own codes.
 */
export async function POST(request) {
  ...
  try {
    ...
    if (!chat) {
      ...
      // Empty patch on purpose: bumps updatedAt so the chat surfaces in recents.
    ...
    const userMessage = await Messages.add({
      ...
      attachments: attachmentList.map((a) => ({ name: a?.name })),
    });
    ...
    console.error("[magicpen] chat turn failed:", err);
    return Response.json(
      { error: { message: "Something went wrong handling this chat. Please try again." } },
      { status: 500 }
    );
  }
}
